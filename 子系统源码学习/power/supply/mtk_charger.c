				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "fast_charging_indicator", &val) >= 0)
		...
		info->fast_charging_indicator = 0x0;
	}
}
						...
						const char *buf, size_t size)
{
	...
	if (kstrtouint(buf, 10, &temp) == 0)
		...
		chr_err("%s: skip fast_charging_indicator set:%d !\n", __func__, temp);
	else
		chr_err("%s: format error!\n", __func__);
...
extern void aw2013_red_led_off(void);
	...
	while (1) {
		...
		if(info->bootmode == 8)
		{
			if(get_uisoc(info) <= 90){
				aw2013_red_led_on();//turn on red light
			}else{
				aw2013_red_led_off();//turn on green light
				aw2013_green_led_on();
			}
		}
