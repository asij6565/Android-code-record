...
int pe_hal_dump_registers(struct chg_alg_device *alg)
{
	struct pe_hal *hal;
...
int pe_hal_enable_charging(struct chg_alg_device *alg, enum chg_idx chgidx, bool enable)
{
	int ret = 0;
	struct pe_hal *hal;
	...
	if (chgidx == CHG1  && hal->chg1_dev != NULL)
		...
		ret = charger_dev_enable(hal->chg2_dev, enable);
...
int pe_hal_set_mivr(struct chg_alg_device *alg, enum chg_idx chgidx, int uV)
{
	...
	if (hal->chg2_dev) {
		ret = charger_dev_set_mivr(hal->chg2_dev, uV + pe->pe_slave_mivr_diff);
		if (ret < 0)
					  ...
					  bool increase)
{
	...
	hal = chg_alg_dev_get_drv_hal_data(alg);
	if (hal->chg2_dev) {
		charger_dev_send_ta_current_pattern(hal->chg2_dev, increase);
	}
	...
	enum chg_idx chgidx, u32 uv)
{
	...
	if (chgidx == CHG1 && hal->chg1_dev != NULL)
		...
		charger_dev_set_constant_voltage(hal->chg2_dev, uv);
...
int pe_hal_is_charger_enable(struct chg_alg_device *alg,
...
int pe_hal_set_eoc_current(struct chg_alg_device *alg,
	enum chg_idx chgidx, u32 uA)
{
	struct pe_hal *hal;
	int ret;
...
int pe_hal_enable_termination(struct chg_alg_device *alg,
...
int pe_hal_get_min_charging_current(struct chg_alg_device *alg,
	enum chg_idx chgidx, u32 *uA)
{
	struct pe_hal *hal;
	int ret;
...
int pe_hal_safety_check(struct chg_alg_device *alg,
	int ieoc)
{
	struct pe_hal *hal;
	...
	hal = chg_alg_dev_get_drv_hal_data(alg);
	charger_dev_safety_check(hal->chg1_dev, ieoc);
	return 0;
}
