...
static const struct pe50_algo_desc algo_desc_defval = {
	...
	.tta_recovery_area = 3,
	.tbat_level_def = {0, 0, 0, 5, 40, 55, 65, 75, 85},
	.tbat_curlmt = {-1, -1, -1, 400, 0, 400, 700, 950, -1},
				     ...
				     u32 ita)
{
	...
	while (true) {
		...
		ret = pe50_hal_set_ta_cap(info->alg, vta, ita);
		if (ret < 0) {
			PE50_ERR("set ta cap fail(%d) but ysj avoid\n", ret);
			ret = pe50_hal_set_ta_cap(info->alg, vta, ita);
			PE50_ERR("set ta cap still ,step 2(%d)\n", ret);
			ret = pe50_hal_set_ta_cap(info->alg, vta, ita);
			PE50_ERR("set ta cap still ,step 3(%d)\n", ret);
			if (ret < 0)	
			return ret;
		}
...
static inline int pe50_get_ita_lmt(struct pe50_algo_info *info)
{
	...
	printk("ysj alarm highte:%s,ita=%d\n",__func__,ita);
	mutex_lock(&data->ext_lock);
	...
	mutex_unlock(&data->ext_lock);
	printk("ysj alarm after highte:%s,ita=%d\n",__func__,ita);
	return ita;
}
...
// 	u32 findbusovp;
// 	ibat = pe50_cal_ibat(info, ita);
// 	printk("ysj_pe5:ibat=%d",ibat);
// 	vout = desc->vbat_cv + div1000(ibat * data->r_sw);
// 	printk("ysj_pe5:vout=%d",vout);
// 	findbusovp= min(percent(pe50_vout2vbus(info, vout), PE50_VBUSOVP_RATIO),
// 		   data->vbusovp);
// 	printk("ysj_pe5:findbusovp=%d",findbusovp);
...
out_set_cap:
	...
	for (attempt = 0; attempt < 3; attempt++)
	{
    	if (ret == -4)
    	{
        	PE50_ERR("PUT TA CAP err SETP %d\n", attempt);
        	msleep(20);
        	ret = pe50_set_ta_cap_cv(info, vta, ita);
    	}
    	else
