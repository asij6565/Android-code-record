...
static struct wakeup_source *droi_suspend_lock = NULL;
...
{
	return PE_DEBUG_LEVEL;
}

static int pe_full_event(struct chg_alg_device *alg)
{
	struct mtk_pe *pe;
	int ret;
	...
	case PE_TUNING:
	case PE_POSTCC:
		pe_hal_is_charger_enable(alg, CHG2, &chg_en);
		chg2_enabled = pe_hal_is_chip_enable(alg, CHG2);

		if (!chg_en || !chg2_enabled) {
			/* notify eoc , fix me */
			pe->state = PE_HW_READY;
			pe_err("charging done:%d %d\n", __func__, chg_en, chg2_enabled);
			if (alg->is_polling_mode == false)
				ret_value = 1;
		} else {
			pe_hal_get_charging_current(alg, CHG2, &ichg2);
			ret = pe_hal_get_min_charging_current(alg, CHG2, &ichg2_min);
			if (ret == -ENOTSUPP)
				ichg2_min = 100000;

			pe_err("ichg2:%d, ichg2_min:%d state:%d\n", ichg2, ichg2_min, pe->state);
			if (ichg2 - 500000 <= ichg2_min) {
				pe->state = PE_POSTCC;
				pe_hal_enable_charging(alg, CHG2, false);
				pe_hal_set_eoc_current(alg, CHG1, 150000);
				pe_hal_enable_termination(alg, CHG1, true);
			} else {
				pe->state = PE_TUNING;
				mutex_lock(&pe->data_lock);
				if (pe->charging_current2 >= 500000)
					pe->charging_current2 = ichg2 - 500000;
				pe_hal_set_charging_current(alg, CHG2, pe->charging_current2);
				mutex_unlock(&pe->data_lock);
			}
			ret_value = 1;
		}
		break;
	default:
		pe_dbg("%s: error state:%d\n", __func__, pe->state);
		break;
	}
...
int mtk_pe_reset_ta_vchr(struct chg_alg_device *alg)
{
	...
	pe = dev_get_drvdata(&alg->dev);
	pe_dbg("%s: starts\n", __func__);
	if (droi_suspend_lock->active) {
		pe_err("%s: droi_suspend unlock\n", __func__);
		__pm_relax(droi_suspend_lock);
	} else {
		pe_err("%s: droi_suspend lock not active\n", __func__);
	}
	...
	do {
		...
		if (chg_cnt > 1) {
			for (i = CHG2; i < CHG_MAX; i++) {
				if (alg->config == DUAL_CHARGERS_IN_SERIES) {
					pe_hal_enable_charging(alg, i, false);
				}
		...
		pe_dbg("%s: retry, cnt = %d, chr_volt = %d, ta_vchr_org = %d\n",
			__func__, retry_cnt, chr_volt,
			pe->ta_vchr_org);
...
static int pe_leave(struct chg_alg_device *alg, bool disable_charging)
{
	...
	ret = pe_hal_enable_charging(alg, CHG1, disable_charging);
	if (ret < 0) {
...
static int pe_increase_ta_vchr(struct chg_alg_device *alg, u32 vchr_target)
{
	...
	pe_dbg("%s: start vchr_target=%d\n", __func__, vchr_target);
	pe_hal_dump_registers(alg);
	pe = dev_get_drvdata(&alg->dev);
	do {
		...
		if (abs(vchr_after - vchr_target) <= VBUS_MAX_DROP) {
			pe_dbg("%s: OK\n", __func__);
			return ret_value;
		} else if (7000000 == vchr_target && abs(vchr_after - 9000000) <= VBUS_MAX_DROP) {
			pe_dbg("%s: OK\n", __func__);
			return ret_value;
		}
	...
	} while (!pe->is_cable_out_occur && pe_hal_get_charger_type(alg) !=
...
static int pe_detect_ta(struct chg_alg_device *alg)
{
	...
	if (pe->ta_vchr_org < 4500000) {
		pe_dbg("%s: ta_vchr_org:%d too low, force 5v\n", __func__, pe->ta_vchr_org);
...
static int pe_plugout_reset(struct chg_alg_device *alg)
{
	...
	if (PE_RUN == pe->state || PE_DONE == pe->state) {
		mutex_lock(&pe->cable_out_lock);
		pe->is_cable_out_occur = true;
		mutex_unlock(&pe->cable_out_lock);

		while (mutex_trylock(&pe->access_lock) == 0) {
			pe_err("%s:pe is running state:%d cnt:%d\n",
				__func__, pe->state, cnt);
			cnt++;
			msleep(100);
		}
...
if (alg->config == DUAL_CHARGERS_IN_SERIES) {
	...
	chg2_chip_enabled = pe_hal_is_chip_enable(alg, CHG2);
	pe_err("chg2_en:%d %d\n", chg2_chip_enabled, pe->state);
	if (pe->state == PE_RUN) {
		if (!chg2_chip_enabled)
			pe_hal_enable_chip(alg, CHG2, true);
		pe_hal_enable_charging(alg, CHG2, true);
		mutex_lock(&pe->data_lock);
		pe->charging_current2 = pe->charging_current / 2;
		mutex_unlock(&pe->data_lock);
		pe_hal_set_input_current(alg, CHG2, pe->input_current / 2);
		pe_hal_set_charging_current(alg, CHG2, pe->charging_current2);

		pe_hal_set_eoc_current(alg, CHG1, pe->dual_polling_ieoc);
		pe_hal_enable_termination(alg, CHG1, false);
		pe_hal_safety_check(alg, pe->dual_polling_ieoc);
	} else if (pe->state == PE_TUNING) {
		if (!chg2_chip_enabled)
			pe_hal_enable_chip(alg, CHG2, true);
		pe_hal_enable_charging(alg, CHG2, true);
		pe_hal_set_eoc_current(alg, CHG1, pe->dual_polling_ieoc);
		pe_hal_enable_termination(alg, CHG1, false);
		pe_hal_safety_check(alg, pe->dual_polling_ieoc);
	} else if (pe->state == PE_POSTCC) {
		pe_hal_set_eoc_current(alg, CHG1, 150000);
		pe_hal_enable_termination(alg, CHG1, true);
	} else {
		pe_err("%s state error!", __func__);
	}
	pe_hal_set_charging_current(alg, CHG1, pe->charging_current / 2);
	pe_hal_set_input_current(alg, CHG1, pe->input_current / 2);
	pe_hal_set_cv(alg, CHG1, pe->cv);
	pe_hal_set_cv(alg, CHG2, pe->cv);
} else {
...
static int _pe_init_algo(struct chg_alg_device *alg)
{
	...
	if (alg->config == DUAL_CHARGERS_IN_SERIES) {
		cnt = pe_hal_get_charger_cnt(alg);
		if (cnt == 2)
...
if (alg->config != DUAL_CHARGERS_IN_SERIES) {
	...
	if (pe->state == PE_RUN ||
		...
		pe->state = PE_HW_READY;
		pe_hal_enable_charging(alg, CHG2, false);
		pe_hal_enable_chip(alg, CHG2, false);
	}
}
...
static int __pe_run(struct chg_alg_device *alg, bool is_reset_ta)
{
	struct mtk_pe *pe;
	...
	if (tune == true) {
		if (is_reset_ta) {
			...
			ret = pe_hal_enable_vbus_ovp(alg, false);
			if (ret < 0)
				pe_err("%s disable vbus ovp fail\n", __func__);
		...
		if (pe->ta_9v_support || pe->ta_12v_support) {
			...
			pe->to_tune_ta_vchr = true;
			if (!droi_suspend_lock->active) {
				__pm_stay_awake(droi_suspend_lock);
				pe_err("%s: droi_suspend lock\n", __func__);
			} else {
				pe_err("%s: droi_suspend already lock\n", __func__);
			}
...
int _pe_start_algo(struct chg_alg_device *alg)
{
	...
	bool again;
	int ret, ret_value;
	bool is_reset_ta = false;

	pe = dev_get_drvdata(&alg->dev);
	pe_dbg("%s state:%d %s, to_tune_ta_vchr:%d\n", __func__,
		pe->state,
		pe_state_to_str(pe->state), pe->to_tune_ta_vchr);

	if (pe->to_tune_ta_vchr && PE_RUN == pe->state) {
		is_reset_ta = true;
	}
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "slave_mivr_diff", &val) >= 0)
		...
		pr_notice("use default slave_mivr_diff:100000\n");
		pe->pe_slave_mivr_diff = 100000;
	}

	if (of_property_read_u32(np, "dual_polling_ieoc", &val) >= 0)
		pe->dual_polling_ieoc = val;
	else {
		pr_notice("use default dual_polling_ieoc :%d\n", 750000);
		pe->dual_polling_ieoc = 750000;
	}
...
static int mtk_pe_probe(struct platform_device *pdev)
{
	...
	droi_suspend_lock = wakeup_source_register(NULL, "PE+ droi suspend wakelock");
	mutex_init(&pe->access_lock);
	mutex_init(&pe->cable_out_lock);
	mutex_init(&pe->data_lock);
