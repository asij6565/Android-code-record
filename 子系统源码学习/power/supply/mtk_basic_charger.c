	...
	struct chg_limit_setting *setting)
{
	...
	if (info->pd_type == MTK_PD_CONNECT_PE_READY_SNK ||
		...
		chr_err("set PD TYPE current");
        if (acap.nr == 1 && acap.max_mv[0] == 5000) {
            pdata->input_current_limit = acap.ma[0];
			chr_err("set PD TYPE current type 1 is %d", pdata->input_current_limit);
        } else {
            pdata->input_current_limit =
			info->data.ac_charger_input_current;
			chr_err("set PD TYPE current type ac");
        }
        pdata->charging_current_limit =
			info->data.ac_charger_current;
		is_basic = true;
...
static int do_algorithm(struct mtk_charger *info)
{
	...
	if (info->is_chg_done != chg_done) {
		...
		if (chg_psy == NULL)
			chg_psy = power_supply_get_by_name("mt6366-charger-type-detection");
		if (chg_psy == NULL || IS_ERR(chg_psy)) {
			chr_err("[droi]%s: Couldn't get chg_psy\n", __func__);
		} else {
